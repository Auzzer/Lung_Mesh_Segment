...
class DeformationProcessor:
    def __init__(self, pts_np, tets_np, labels_np):
        ...
        # r_axis[k, ell] : 12-dim row for ε_ell,ell  (ell=0,1,2)
        self.r_axis  = ti.Vector.field(12, ti.f64, shape=(self.M, 3))
        # r_shear[k, s] : 12-dim row for ε_ell,m (s=0:(0,1), 1:(1,2), 2:(2,0))
        self.r_shear = ti.Vector.field(12, ti.f64, shape=(self.M, 3))
        # r_vol[k] : 12-dim row for tr(ε)  via δV/V
    ...
    def _initialize_volume_kappa(self):
        ...
        for k in range(self.M):
            base=1.0
            if self.label[k] == 1: # air-rich
                base *= 0.2
            elif self.label[k] == 2: # fibrotic
                base *= 2.0
            self.volume_kappa[k] = base
...
def load_mesh_with_displacement(mesh_path, displacement_path=None):
    ...
    from old_versions.displacement_loader import load_and_interpolate_displacement
