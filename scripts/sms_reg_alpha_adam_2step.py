...
# Trust-region style caps for optimizer steps in log-space
MAX_DTHETA = 0.5
MAX_DDELTA = 0.25
...
def main():
    ...
    parser.add_argument("--max-dtheta", type=float, default=0.5, help="Step cap for theta (log space)")
    parser.add_argument("--max-ddelta", type=float, default=0.25, help="Step cap for delta field")
    parser.add_argument(
        "--max-iters",
        type=int,
        default=100,
        help="Number of Adam outer iterations for warp mode",
    )
    parser.add_argument("--stage1-max-iters", type=int, default=1, help="Maximum number of Stage 1 (theta) Adam iterations")
    ...
    def run_warp_style():
        ...
        prev_loss = None
        warp_state: dict[str, object] = {}

        log("[Warp] starting Adam optimization (single-stage)")
        for iteration in range(1, max_iters + 1):
            ...
            loss_display = warp_state.get("loss_total")
            log(
                f"[Warp] Iter {iteration:03d}: loss={loss_display:.6e}, "
                f"loss_data={warp_state.get('loss_data', 0.0):.6e}, "
                f"loss_reg={warp_state.get('loss_reg', 0.0):.6e}, "
                f"alpha_mean={warp_state.get('alpha_mean', 0.0):.4e}, "
                f"alpha_std={warp_state.get('alpha_std', 0.0):.4e}, "
                f"|grad_theta|={abs(warp_state.get('grad_theta', 0.0)):.4e}, "
                f"grad_delta_mean={warp_state.get('grad_delta_mean', 0.0):.4e}, "
            ...
            history_entry = {
                ...
                "loss_data": warp_state.get("loss_data"),
                "loss_reg": warp_state.get("loss_reg"),
                "alpha_mean": warp_state.get("alpha_mean"),
                "alpha_std": warp_state.get("alpha_std"),
                "grad_theta": warp_state.get("grad_theta"),
                "grad_delta_mean": warp_state.get("grad_delta_mean"),
                "reg_weight": reg_weight,
                "forward_status": warp_state.get("fw_status"),
                "backward_status": warp_state.get("bw_status"),
            }
            if warp_state.get("alpha_mae") is not None:
                history_entry["alpha_mae"] = warp_state["alpha_mae"]
            history_entries.append(history_entry)

            if prev_loss is not None and loss_display is not None:
    ...
    def run_two_step_optimization():
        ...
        while coarse_iter < coarse_max_iters and not coarse_converged:
            ...
            log("[Global-Coarse] assembling stiffness matrix (step)")
            sim.assemble_matrix(alpha_field, beta_field, kappa_field)
            log("[Global-Coarse] forward solve (step)")
            sim.forward(tol=1e-6, max_iter=200)
            ...
            prev_loss = coarse_prev_loss
            coarse_prev_loss = coarse_state['loss_total']
            if prev_loss is not None:
                delta_loss = abs(prev_loss - coarse_state['loss_total'])
                if delta_loss <= loss_nochange_tol * max(1.0, abs(prev_loss)):
                    log("[Global-Coarse] Loss unchanged within tolerance; stopping.")
            ...
            if loss_value is None:
                log("[Global-Coarse] Step produced no loss; stopping.")
                coarse_converged = True
            else:
        ...
        optimizer_theta = torch.optim.Adam([theta_param], lr=initial_lr)
        optimizer_delta = torch.optim.Adam([delta_param], lr=initial_lr)
        ...
        global_prev_loss = None
        reg_weight_state = {"value": REG_WEIGHT_WARMUP}

        log("[Global-Fine] starting sequential optimization")
        for iter_idx in range(1, num_iterations + 1):
            theta_warmup_active = iter_idx <= WARMUP_GLOBAL_ITERS
            ...
            log("[Global-Fine] assembling stiffness matrix (step)")
            sim.assemble_matrix(alpha_field, beta_field, kappa_field)
            log("[Global-Fine] forward solve (step)")
            sim.forward(tol=1e-6, max_iter=200)
            ...
            global_state['loss_total'] = loss_value
            global_state['loss_data'] = float(loss_data.item())
            global_state['loss_reg'] = float(loss_reg.item())
            global_state['alpha_mean'] = float(alpha_field.mean().item())
            global_state['alpha_std'] = float(alpha_field.std().item())
            global_state['grad_norm'] = float(abs(grad_theta.item()))
            global_state['fw_status'] = sim._last_forward_status
            global_state['bw_status'] = sim._last_backward_status
            if alpha_gt_torch is not None:
                global_state['alpha_mae'] = float(
                    torch.mean(torch.abs(alpha_field - alpha_gt_torch)).item()
                )
            else:
                global_state['alpha_mae'] = None

            prev_loss = global_prev_loss
            ...
            stop_global = False
            if prev_loss is not None:
                delta_loss = abs(prev_loss - loss_value)
                if delta_loss <= loss_nochange_tol * max(1.0, abs(prev_loss)):
                    log("[Global-Fine] Loss unchanged within tolerance; stopping.")
                    stop_global = True

            theta_update = float(torch.max(torch.abs(theta_param.detach() - prev_theta)).item())
            alpha_eval = broadcast_alpha(theta_param.detach(), delta_param.detach())
            lr_theta = float(optimizer_theta.param_groups[0]["lr"])
            theta_value = float(theta_param.detach().item())
            iter_display = global_iter_offset + iter_idx
            log(
                f"[Global-Fine] iter {iter_display:03d}: loss={loss_value:.6e}, "
                f"alpha_mean={alpha_eval.mean().item():.4e}, alpha_std={alpha_eval.std().item():.4e}, "
                f"grad_theta={global_state['grad_norm']:.4e}, theta={theta_value:.6e}, "
            ...
            history_entry = {
                ...
                "phase": "iterate",
                "loss_total": loss_value,
                "loss_data": global_state['loss_data'],
                "loss_reg": global_state['loss_reg'],
                "alpha_mean": global_state['alpha_mean'],
                "alpha_std": global_state['alpha_std'],
                "grad_theta_norm": global_state['grad_norm'],
                "theta_value": theta_value,
                "param_update": theta_update,
                "learning_rate": lr_theta,
                "forward_status": global_state['fw_status'],
                "backward_status": global_state['bw_status'],
            }
            if global_state['alpha_mae'] is not None:
                history_entry['alpha_mae'] = global_state['alpha_mae']
            history_global.append(history_entry)
        ...
        local_prev_loss = None

        log("[Local-Fine] starting sequential optimization")
        for iter_idx in range(1, num_iterations + 1):
            current_ddelta_cap = warmup_ddelta_cap if iter_idx <= WARMUP_GLOBAL_ITERS else base_max_ddelta
            reg_weight_state['value'] = REG_WEIGHT_TARGET
            ...
            log("[Local-Fine] assembling stiffness matrix (step)")
            sim.assemble_matrix(alpha_field, beta_field, kappa_field)
            log("[Local-Fine] forward solve (step)")
            sim.forward(tol=1e-6, max_iter=200)
            ...
            loss_value = float(combined_loss.item())
            local_state['loss_total'] = loss_value
            local_state['loss_data'] = float(loss_data.item())
            local_state['loss_reg'] = float(loss_reg.item())
            local_state['loss_param'] = float(shrink_loss.item())
            local_state['alpha_mean'] = float(alpha_field.mean().item())
            local_state['alpha_std'] = float(alpha_field.std().item())
            local_state['grad_norm'] = float(g_delta.abs().mean().item())
            local_state['fw_status'] = sim._last_forward_status
            local_state['bw_status'] = sim._last_backward_status
            if alpha_gt_torch is not None:
                local_state['alpha_mae'] = float(
                    torch.mean(torch.abs(alpha_field - alpha_gt_torch)).item()
                )
            else:
                local_state['alpha_mae'] = None

            prev_loss = local_prev_loss
            ...
            if prev_loss is not None:
                delta_loss = abs(prev_loss - loss_value)
                if delta_loss <= loss_nochange_tol * max(1.0, abs(prev_loss)):
                    log("[Local-Fine] Loss unchanged within tolerance; stopping.")
                    stop_local = True
            ...
            grad_delta_mean = float(local_state['grad_norm'])
            log(
                f"[Local-Fine] iter {iter_idx:03d}: loss={loss_value:.6e}, "
                f"alpha_mean={alpha_eval.mean().item():.4e}, alpha_std={alpha_eval.std().item():.4e}, "
                f"grad_delta_mean={grad_delta_mean:.4e}, delta_mean={delta_mean:.4e}, "
                f"delta_std={delta_std:.4e}, delta_rms={delta_rms:.4e}, "
                f"loss_param={local_state['loss_param']:.4e}, "
            ...
            history_entry = {
                ...
                "phase": "iterate",
                "loss_total": loss_value,
                "loss_data": local_state['loss_data'],
                "loss_reg": local_state['loss_reg'],
                "loss_param": local_state['loss_param'],
                "alpha_mean": local_state['alpha_mean'],
                "alpha_std": local_state['alpha_std'],
                ...
                "forward_status": local_state['fw_status'],
                "backward_status": local_state['bw_status'],
            }
            if local_state['alpha_mae'] is not None:
                history_entry['alpha_mae'] = local_state['alpha_mae']
        ...
        history_global.append({
            ...
            "phase": "final",
            "loss_total": global_state.get('loss_total'),
        ...
        history_local.append({
            ...
            "phase": "final",
            "loss_total": local_state.get('loss_total'),
            "loss_param": local_state.get('loss_param'),
            "alpha_mean": float(alpha_final_field.mean().item()),
            "alpha_std": float(alpha_final_field.std().item()),
    ...
    if args.mode == "warp":
        ...
        theta_final, delta_final, alpha_final_field = run_two_step_optimization()
