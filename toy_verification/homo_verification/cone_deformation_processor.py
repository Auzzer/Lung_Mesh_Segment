...
1. Loading original cone mesh from cone.xdmf
2. Loading deformed cone mesh from cone_droop.xdmf (FEM result)
3. Computing displacement field as the difference
4. Processing deformation analysis using the same pipeline as deformation_processor_v2.py
5. Saving results for use with Static_Equilibrium_Simplified_V2_scipy_version.py

This enables verification of the SMS (Second Moment of Stiffness) approach against
standard FEM linear elasticity results.

KEY MODIFICATIONS FROM ORIGINAL:
- Adapted for cone geometry instead of lung tissue
- Uses FEM displacement field instead of 4DCT registration data
- Simplified material properties (uniform cone material)
...
# Physical constants for cone (matching FEM simulation)
# Simplified from lung tissue constants to uniform cone material
DEFAULT_RHO_CONE = 200.0  # kg/m³ (matching homo_cone_fem.py)
DEFAULT_KAPPA = 14285.7143
...
class ConeDeformationProcessor:
    ...
    def _initialize_data(self, pts_np, tets_np, labels_np):
        ...
        # MODIFICATION: Positions are already in meters from FEM simulation (not mm like lung data)
        ...
        # MODIFICATION: Set uniform density for cone material (instead of heterogeneous lung tissue)
        rho_np = np.full(self.M, DEFAULT_RHO_CONE, dtype=np.float64)
        ...
        for k in range(self.M):
            if self.is_degenerate[k] == 0:
                ...
                if ti.abs(det_D_m) > EPS_ZERO:
                    ...
                    # Compute deformation coefficients: C_ij = F_ik * (nablaN_j)_k
    ...
    def _initialize_stiffness(self):
        """Initialize stiffness parameters - uniform for cone material"""
        # MODIFICATION: Uniform stiffness instead of tissue-dependent values
        for k in range(self.M):
            # Base stiffness scaled for cone material properties
            base_stiffness = 1000.0  # N/m (stiffer than lung tissue)
    ...
    def _initialize_torsion_springs(self):
        ...
        for k in range(self.M):
            if self.vol[k] > 1e-10:
                base_torsion_stiffness = 100.0  # N⋅m/rad (appropriate for cone)
                ...
                self.rest_cos_angles[k, 0] = 0.0
                self.rest_cos_angles[k, 1] = 0.0
                self.rest_cos_angles[k, 2] = 0.0
    
    @ti.kernel
    def _initialize_volume_kappa(self):
        """Initialize bulk modulus for each tetrahedron"""
        for k in range(self.M):
            # Uniform bulk modulus for cone material
            self.volume_kappa[k] = DEFAULT_KAPPA
    ...
    def _detect_boundary_nodes(self):
        ...
        # MODIFICATION: Cone-specific boundary detection instead of lung surface detection
    ...
    def get_results(self):
        ...
        results = {
            ...
            'volume_kappa': self.volume_kappa.to_numpy(),
...
def load_cone_mesh_data(original_mesh_path, deformed_mesh_path):
    ...
    # MODIFICATION: Loads FEM results instead of 4DCT displacement data
    ...
    for cell in original_mesh.cells:
        if hasattr(cell, 'type') and cell.type == "tetra":
            tets = cell.data
            break
    if tets is None:
    ...
    # MODIFICATION: Compute displacement field from FEM solution difference
    # instead of interpolating from medical imaging data
    displacement_vectors = deformed_pts - original_pts
    
    # MODIFICATION: Create uniform labels (cone has single material type)
    labels = np.zeros(tets.shape[0], dtype=np.int32)
...
def process_cone_verification():
    """Process cone verification case"""
    import os
    
    # Paths for cone mesh files
    original_mesh_path = "homo_verification/cone.xdmf"
    deformed_mesh_path = "homo_verification/cone_droop.xdmf"
    output_path = "homo_verification/cone_verification_deformation"
    
    print("=== Cone Verification Deformation Processing ===")
    
    # Check if files exist
    if not os.path.exists(original_mesh_path):
        print(f"Error: Original cone mesh not found at {original_mesh_path}")
        return
    if not os.path.exists(deformed_mesh_path):
        print(f"Error: Deformed cone mesh not found at {deformed_mesh_path}")
        return
    
    # Load cone mesh data
    pts, tets, lbls, displacement_vectors = load_cone_mesh_data(
        original_mesh_path, deformed_mesh_path
    )
    
    # Create processor
    print("Creating cone deformation processor...")
    processor = ConeDeformationProcessor(pts, tets, lbls)
    ...
    print("Ready for use with Static_Equilibrium_Simplified_V2_scipy_version.py")
