"""
Cone Deformation Processor for Homogenization Verification

MODIFIED FROM: deformation_processor_v2.py
PURPOSE: Cone-specific verification case for SMS homogenization validation

This script processes the cone mesh verification case by:
1. Loading original cone mesh from xdmf_visualization/cone_fem_initial.xdmf
2. Loading deformed cone mesh from xdmf_visualization/cone_fem_droop.xdmf (FEM result)
3. Loading exact material properties from cone_data/cone_fem_droop.npz
4. Computing displacement field as the difference
5. Processing deformation analysis using the same pipeline as deformation_processor_v2.py
6. Saving results for use with hetero_cone_static_equilibrium.py

This enables verification of the SMS (Second Moment of Stiffness) approach against
standard FEM linear elasticity results with heterogeneous material properties.

KEY MODIFICATIONS FROM ORIGINAL:
- Adapted for cone geometry instead of lung tissue
- Uses FEM displacement field instead of 4DCT registration data
- Uses exact heterogeneous material properties (mu, lambda) from FEM
- Modified boundary detection for cone top face constraint
- Removed lung-specific tissue type handling

MATERIAL PROPERTY CALCULATION:
From FEM data (E, nu, mu, lambda), SMS parameters are computed as:
- alpha = 0.5 * mu  (axial stiffness coefficient)
- beta = mu         (shear stiffness coefficient)
- kappa = lambda    (volumetric stiffness coefficient)

These values are directly extracted from the FEM simulation's Lamé parameters.
"""
...
@ti.data_oriented
class ConeDeformationProcessor:
    def __init__(
        self,
        pts_np,
        tets_np,
        ...
        self.displacement_field = ti.Vector.field(3, ti.f64, shape=self.N)        # u_i from FEM
        self.deformed_positions = ti.Vector.field(3, ti.f64, shape=self.N)        # x_i = X_i + u_i
        self.deformation_gradient = ti.Matrix.field(3, 3, ti.f64, shape=self.M)   # F matrix per tet
        ...
        self.alpha_k = ti.field(ti.f64, shape=self.M)
        self.beta_k = ti.field(ti.f64, shape=self.M)
        self.kappa_k = ti.field(ti.f64, shape=self.M)
        ...
        # SMS quadratic measurement rows (reference config)
        ...
        # Cache provided heterogeneous parameters
        self._rho_input = rho_np
        self._alpha_input = alpha_np
        self._beta_input = beta_np
        self._kappa_input = kappa_np
        self.nu_value = float(nu_value) if nu_value is not None else None
    ...
    def _initialize_data(self, pts_np, tets_np, labels_np):
        ...
        #  Positions are already in meters from FEM simulation (not mm like lung data)
        ...
        self.rho.from_numpy(np.asarray(self._rho_input, dtype=np.float64))
        self.alpha_k.from_numpy(np.asarray(self._alpha_input, dtype=np.float64))
        self.beta_k.from_numpy(np.asarray(self._beta_input, dtype=np.float64))
        self.kappa_k.from_numpy(np.asarray(self._kappa_input, dtype=np.float64))
    ...
    def set_displacement_field(self, displacement_np):
        """Set displacement field from FEM results"""
        ...
        # construct SMS measurement rows after calculating anisotropy axes and intersections
        self._build_sms_measure_rows()
    ...
    def _compute_edge_matrices(self):
        ...
        for k in range(self.M):
            ...
            X0 = self.initial_positions[v0_idx]; X1 = self.initial_positions[v1_idx]
            X2 = self.initial_positions[v2_idx]; X3 = self.initial_positions[v3_idx]

            # Deformed positions
            x0 = self.deformed_positions[v0_idx]; x1 = self.deformed_positions[v1_idx]
            x2 = self.deformed_positions[v2_idx]; x3 = self.deformed_positions[v3_idx]

            # Compute edge vectors from vertex 0
            # D_m = [X1-X0, X2-X0, X3-X0] (3x3 matrix)
            edge1_ref = X1 - X0; edge2_ref = X2 - X0; edge3_ref = X3 - X0
            edge1_def = x1 - x0; edge2_def = x2 - x0; edge3_def = x3 - x0
    ...
    def _compute_deformation_gradients(self):
        """Compute deformation gradient F = d_x · D_m^(-1) for each tetrahedron"""
        for k in range(self.M):
            ...
            if ti.abs(det_D_m) > EPS_ZERO:
                ...
                F = d_x @ D_m_inv
                self.deformation_gradient[k] = F
                
                # Check for degenerate deformation (negative determinant)
                det_F = F.determinant()
                self.is_degenerate[k] = 1 if det_F <= 0.0 else 0
    ...
    def _compute_anisotropy_axes_from_svd(self):
        """Compute anisotropy axes from deformation gradient using SVD: F = U·Sigma·V^T"""
        for k in range(self.M):
            if self.is_degenerate[k] == 0:
                # Get deformation gradient F
                F = self.deformation_gradient[k]
                
                # Perform SVD: F = U·Sigma·V^T
                U, sigma, V = ti.svd(F)
                ...
                # Store principal stretches (sorted lambda_0 >= lambda_1 >= lambda_2)
    ...
    def _handle_degenerate_tetrahedron(self, k):
        ...
        # Get vertex indices
        v0_idx = self.tets[k][0]; v1_idx = self.tets[k][1]; v2_idx = self.tets[k][2]; v3_idx = self.tets[k][3]
        ...
        # Initialize axes
        e0 = ti.Vector([1.0, 0.0, 0.0]); e1 = ti.Vector([0.0, 1.0, 0.0]); e2 = ti.Vector([0.0, 0.0, 1.0])
        ...
        if u_bar_norm > eps_displacement:
            ...
            # Third axis: e2 = e0 x e1
            e2 = e0.cross(e1)
        
        # Set anisotropy axes
        self.anisotropy_axes[k, 0] = e0; self.anisotropy_axes[k, 1] = e1; self.anisotropy_axes[k, 2] = e2
        ...
        for k in range(self.M):
            if self.is_degenerate[k] == 0:
                ...
                if ti.abs(det_D_m) > EPS_ZERO:
                    ...
                    # Compute deformation coefficients: C_ij = F_ik * (nabla N_j)_k
                    for i in ti.static(range(3)):  # spatial directions
                        self.deformation_coeffs[k][i, 0] = (F[i, 0] * grad_N0[0] + 
                                                           F[i, 1] * grad_N0[1] + 
                                                           F[i, 2] * grad_N0[2])
                        self.deformation_coeffs[k][i, 1] = (F[i, 0] * grad_N1[0] + 
                                                           F[i, 1] * grad_N1[1] + 
                                                           F[i, 2] * grad_N1[2])
                        self.deformation_coeffs[k][i, 2] = (F[i, 0] * grad_N2[0] + 
                                                           F[i, 1] * grad_N2[1] + 
                                                           F[i, 2] * grad_N2[2])
                        self.deformation_coeffs[k][i, 3] = (F[i, 0] * grad_N3[0] + 
                                                           F[i, 1] * grad_N3[1] + 
                                                           F[i, 2] * grad_N3[2])
                else:
    ...
    @ti.kernel
    def _build_sms_measure_rows(self):
        """
        Build linear measurement rows r for axis, shear and volumetric terms on the REFERENCE config.
        q = r * u_e, where u_e is 12-dof local displacement vector.
        """
        for k in range(self.M):
            if self.vol[k] <= 1e-12:
                # Degenerate element: zero out measurements
                for ell in ti.static(range(3)):
                    self.r_axis[k, ell].fill(0.0)
                    self.r_shear[k, ell].fill(0.0)
                self.r_vol[k].fill(0.0)
                continue

            V0 = self.vol[k]
            # volume row r_vol, axis rows r_axis, shear rows r_shear
            # --- Volume row r_vol = stack(g_a / V0) ---
            # Compute volume gradients g_a using reference vertices
            i0, i1, i2, i3 = self.tets[k][0], self.tets[k][1], self.tets[k][2], self.tets[k][3]
            X0, X1, X2, X3 = self.initial_positions[i0], self.initial_positions[i1], self.initial_positions[i2], self.initial_positions[i3]
            ...
            rv = ti.Vector([0.0]*12)
            G = [g0/V0, g1/V0, g2/V0, g3/V0]
            for a in ti.static(range(4)):
                rv[3*a+0], rv[3*a+1], rv[3*a+2] = G[a][0], G[a][1], G[a][2]
            self.r_vol[k] = rv

            # --- Axis/shear rows (based on reference intersections and principal axes) ---
            e0 = self.anisotropy_axes[k, 0]
            e1 = self.anisotropy_axes[k, 1]
            e2 = self.anisotropy_axes[k, 2]

            # Precompute per-axis barycentric difference s_ell[a]=C[a,pt2]-C[a,pt1] and L0
            s_vec = ti.Matrix.zero(ti.f64, 3, 4)   # 3 axes x 4 nodes
            L0    = ti.Vector([0.0, 0.0, 0.0])
            for ell in ti.static(range(3)):
                pt1 = 2*ell
                pt2 = 2*ell + 1
                if self.intersection_valid[k, pt1] == 1 and self.intersection_valid[k, pt2] == 1:
                    p1 = self.intersection_points[k, pt1]
                    p2 = self.intersection_points[k, pt2]
                    L0[ell] = (p1 - p2).norm()
                    for a in ti.static(range(4)):
                        s_vec[ell, a] = self.C_k[k, a, pt2] - self.C_k[k, a, pt1]
                else:
                    L0[ell] = 0.0
                    for a in ti.static(range(4)):
                        s_vec[ell, a] = 0.0

            # r_axis: q_ll = (e_l^T * sum_a s_l[a] u_a) / L0_l
            for ell in ti.static(range(3)):
                r = ti.Vector([0.0]*12)
                if L0[ell] > 1e-12:
                    # Select axis vector n by compile-time index
                    n = e0
                    if ell == 1:
                        n = e1
                    elif ell == 2:
                        n = e2
                    for a in ti.static(range(4)):
                        coef = s_vec[ell, a] / L0[ell]
                        r[3*a+0] += coef * n[0]
                        r[3*a+1] += coef * n[1]
                        r[3*a+2] += coef * n[2]
                self.r_axis[k, ell] = r

            # r_shear: q_lm = 0.5*( (e_m^T sum_a s_l[a] u_a)/L0_l + (e_l^T sum_a s_m[a] u_a)/L0_m )
            for s in ti.static(range(3)):
                # Determine pair (L, M) for s=0,1,2
                L = 0; M = 1
                if s == 1:
                    L = 1; M = 2
                elif s == 2:
                ...
                # Select axis vectors for L and M
                nL = e0; nM = e0
                if L == 1:
                    nL = e1
                elif L == 2:
                ...
                r = ti.Vector([0.0]*12)
                if L0[L] > 1e-12 or L0[M] > 1e-12:
                    for a in ti.static(range(4)):
                        if L0[L] > 1e-12:
                            coef = 0.5 * (s_vec[L, a] / L0[L])
                            r[3*a+0] += coef * nM[0]
                            r[3*a+1] += coef * nM[1]
                            r[3*a+2] += coef * nM[2]
                        if L0[M] > 1e-12:
                            coef = 0.5 * (s_vec[M, a] / L0[M])
                            r[3*a+0] += coef * nL[0]
                            r[3*a+1] += coef * nL[1]
                            r[3*a+2] += coef * nL[2]
                self.r_shear[k, s] = r
    ...
    def _detect_boundary_nodes(self):
        """Detect boundary nodes for the cone geometry"""
        #  Cone-specific boundary detection instead of lung surface detection
        # Initialize boundary detection
        self._initialize_boundary_detection()
        
        # For cone, we want to constrain the top face (max Z)
        self._mark_cone_boundary_nodes()
        
        boundary_count = self._count_boundary_nodes()
        print(f"Cone boundary detection: {boundary_count} boundary nodes detected")
    ...
    def _mark_cone_boundary_nodes(self):
        """Mark boundary nodes for the cone - constrain the top face"""
        positions = self.x.to_numpy()
        z_coords = positions[:, 2]
        z_max = float(z_coords.max())
        
        # Nodes near the maximum Z coordinate are boundary nodes (top face)
        top_tol = 1e-4
        boundary_mask = np.abs(z_coords - z_max) < top_tol
        
        # Convert to Taichi format
        boundary_nodes_array = np.where(boundary_mask)[0].astype(np.int32)
    ...
    def get_results(self):
        ...
        if self.nu_value is not None:
            results['nu'] = float(self.nu_value)
        return results
    ...
    def save_results(self, output_path):
        ...
        print(f"Cone verification results saved to {output_path}.npz")
    
    def print_summary(self):
        """Print summary of deformation analysis for cone"""
        print(f"\n=== Cone Deformation Analysis Summary ===")
        ...
        print(f"  Displacement std: {displacement_magnitudes.std():.6f} m")
...
def load_cone_mesh_data(original_mesh_path, deformed_mesh_path):
    """
    Load cone mesh data and compute displacement field from FEM results.
    
    Loads initial and deformed meshes from FEM simulation and computes 
    displacement vectors as the difference between deformed and initial positions.
    
    Args:
        original_mesh_path: Path to initial mesh (cone_fem_initial.xdmf)
        deformed_mesh_path: Path to deformed mesh (cone_fem_droop.xdmf)
    
    Returns:
        original_pts: Initial vertex positions
        tets: Tetrahedron connectivity
        labels: Region labels per tetrahedron
        displacement_vectors: Displacement field (deformed - original)
    """
    print(f"Loading original cone mesh: {original_mesh_path}")
    original_mesh = meshio.read(original_mesh_path)
    
    print(f"Loading deformed cone mesh: {deformed_mesh_path}")
    deformed_mesh = meshio.read(deformed_mesh_path)
    
    # Extract points and tetrahedra
    original_pts = original_mesh.points
    deformed_pts = deformed_mesh.points
    
    tets = original_mesh.get_cells_type("tetra")
    if tets is None or tets.size == 0:
        raise ValueError("No tetrahedral cells found in cone mesh")

    label_data = None
    if "labels" in original_mesh.cell_data:
        label_data = original_mesh.cell_data["labels"][0]
    else:
        cell_dict = original_mesh.cell_data_dict.get("tetra", {})
        if "labels" in cell_dict:
            label_data = cell_dict["labels"]
    if label_data is None:
        raise ValueError("Original mesh missing 'labels' cell data")

    labels = np.asarray(label_data, dtype=np.int32)
    if labels.shape[0] != tets.shape[0]:
        raise ValueError("Label count does not match tetrahedra count")

    displacement_vectors = deformed_pts - original_pts

    print(f"Cone mesh loaded: {original_pts.shape[0]} nodes, {tets.shape[0]} tetrahedra")
    print(f"Label stats: mean={labels.mean():.4f}, special={labels.sum()} cells")
    print(f"Max displacement magnitude: {np.linalg.norm(displacement_vectors, axis=1).max():.6e} m")
    
    return original_pts, tets, labels, displacement_vectors


def process_cone_verification():
    """Process cone verification case using exact FEM material properties"""
    ...
    print("=== Cone Verification Deformation Processing ===")
    ...
    pts, tets, lbls, displacement_vectors = load_cone_mesh_data(
    ...
    # Extract exact Lamé parameters from FEM
    ...
    # Verify labels match
    stored_labels = fem_data["labels"].astype(np.int32)
    if stored_labels.shape[0] != lbls.shape[0] or not np.array_equal(stored_labels, lbls):
        print("Warning: Label mismatch between mesh and FEM data; using mesh labels")

    # Compute SMS material parameters from exact FEM Lamé parameters
    alpha_cells = 0.5 * mu_cells  # Axial stiffness
    beta_cells = mu_cells          # Shear stiffness  
    kappa_cells = lambda_cells     # Volumetric stiffness
    
    print(f"\nMaterial properties loaded from FEM:")
    print(f"  mu (shear modulus):     {mu_cells.mean():.2f} ± {mu_cells.std():.2f} Pa")
    print(f"  lambda (Lamé 1st):      {lambda_cells.mean():.2f} ± {lambda_cells.std():.2f} Pa")
    print(f"  rho (density):          {rho_cells.mean():.2f} ± {rho_cells.std():.2f} kg/m³")
    print(f"\nSMS parameters computed:")
    print(f"  alpha = 0.5*mu:         {alpha_cells.mean():.2f} ± {alpha_cells.std():.2f} Pa")
    print(f"  beta = mu:              {beta_cells.mean():.2f} ± {beta_cells.std():.2f} Pa")
    print(f"  kappa = lambda:         {kappa_cells.mean():.2f} ± {kappa_cells.std():.2f} Pa")

    print("\nCreating cone deformation processor...")
    processor = ConeDeformationProcessor(
        pts,
        tets,
        lbls,
        rho_np=rho_cells,
    ...
    print("Computing deformation analysis...")
    ...
    print("\n=== Cone Verification Complete ===")
    print(f"Results saved to: {output_path}.npz")
    print("Ready for use with hetero_cone_static_equilibrium.py")
    

if __name__ == "__main__":
    process_cone_verification()
