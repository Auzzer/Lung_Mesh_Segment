...
\begin{frame}[t]
  ...
  \begin{columns}[T,onlytextwidth]
    \column{0.66\textwidth}
      \begin{minipage}[t]{0.96\linewidth}\small
      \textbf{A Simple Spring--Mass Model (2D cloth as intuition)}\\[0.5em]
      \textbf{Mass points \& springs.} Cloth meshes (2D) are often modeled with masses at vertices and linear springs on edges. \textbf{Stretch} springs penalize edge-length changes; \textbf{shear} springs penalize angle change between warp/weft; optional \textbf{bending} springs penalize dihedral changes along folds \cite{bourguignon2000anisotropy,lakhal2013modified}.\\[0.5em]
    ...
    \column{0.28\textwidth}
      \vspace*{\fill}
      \centering
      \includegraphics[width=1.2\linewidth]{fig2.0.png}
  \end{columns}
  Differentiation gives \textbf{internal forces} that are linear in displacements around the rest state.\\
  Our 3D \textbf{SMS (for tetrahedra)} follows the same idea: 
  we define simple measurements of deformation along a few directions and 
  build a \textbf{quadratic energy}, whose gradient gives internal forces 
  and whose Hessian gives a \textbf{global stiffness}. 
...
\begin{frame}
  ...
  Now we use the previous intuition to 3D tets with anisotropy.
  \begin{itemize}
