...
- displacement Charbonnier-TV regularization (same as sms_reg_u_lbfgs.py)
"""
...
def main():
    parser = argparse.ArgumentParser(description="SMS inverse problem (direct per-tet alpha with displacement-TV regularization).")
    ...
    ct_fallback = None
    if args.ct_path:
        ct_fallback = Path(args.ct_path)
    elif example_paths.get("fixed_image"):
    ...
    from scripts.sms_reg_u_lbfgs import _ensure_taichi_initialized  # local import to reuse existing init
    ...
    for it in range(1, max_iters + 1):
        ...
        if len(loss_history) >= 2 and abs(loss_history[-1] - loss_history[-2]) <= 1e-6 * max(1.0, abs(loss_history[-2])):
    ...
    from scripts.sms_reg_u_lbfgs import save_parameter_heatmap as save_heatmap  # reuse existing helper

    save_heatmap(sim, alpha_final_field, save_path, labels_np, log_fn=log)
