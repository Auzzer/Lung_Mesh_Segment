...
Only alpha is the optimization parameter
So beta and kappa are derived from alpha using the relationships:
1. beta = 2*alpha (since beta = mu = 2*alpha)
2. kappa = 4*alpha*nu/(1-2*nu) (since kappa = lambda, and lambda = E*nu/((1+nu)(1-2nu)) with E = 4*alpha*(1+nu))
Then the gradient with chain rule: 
dL/dalpha_total 
=dL/dalpha + dL/dbeta * dbeta/dalpha + dL/dkappa * dkappa/dalpha
=dL/dalpha + 2dL/dbeta +  4*nu/(1-2*nu)*dL/dkappa
...
class ConeStaticEquilibrium:
    ...
    def _grad_params_kernel(self,
                            grad_alpha_total: ti.types.ndarray(),   # Total gradient for alpha including chain rule
                            grad_alpha_direct: ti.types.ndarray(),  # Direct contribution (for diagnostics)
                            grad_beta_contribution: ti.types.ndarray(),   # Beta contribution (for diagnostics)
                            grad_kappa_contribution: ti.types.ndarray(),  # Kappa contribution (for diagnostics)
                            u_free:     ti.types.ndarray(),   # (n_free,)
                            lam_free:   ti.types.ndarray(),   # (n_free,)
                            nu: ti.f64                        # Poisson's ratio
                            ):
        # Chain rule coefficients for alpha:
        # beta = 2*alpha => dbeta/dalpha = 2
        # kappa = 4*alpha*nu/(1-2*nu) => dkappa/dalpha = 4*nu/(1-2*nu)
        coeff_beta = 2.0
        ...
        for k in range(self.M):
            V = self.vol[k]
            if V <= 1e-12:
                grad_alpha_total[k] = 0.0
                grad_alpha_direct[k] = 0.0
                grad_beta_contribution[k] = 0.0
                grad_kappa_contribution[k] = 0.0
                continue
            ...
            # Compute individual gradients with formulas:
            ...
            # Store individual contributions for diagnostics
            grad_alpha_direct[k] = g_alpha
            grad_beta_contribution[k] = coeff_beta * g_beta
            grad_kappa_contribution[k] = coeff_kappa * g_kappa
            
            # Apply chain rule to compute total gradient for alpha:
            # dL/dalpha_total = dL/dalpha + (dL/dbeta)*(dbeta/dalpha) + (dL/dkappa)*(dkappa/dalpha)
            grad_alpha_total[k] = g_alpha + coeff_beta * g_beta + coeff_kappa * g_kappa
    ...
    def backward(self,
        ...
        Returns:
            ...
            grad_alpha_total: total gradient for alpha (with chain rule applied)
            grad_alpha_direct: direct contribution (for diagnostics)
            grad_beta_contrib: beta contribution (for diagnostics)
            grad_kappa_contrib: kappa contribution (for diagnostics)
        """
        ...
        grad_alpha_total = torch.zeros(self.M, device='cuda', dtype=torch.float64)
        grad_alpha_direct = torch.zeros(self.M, device='cuda', dtype=torch.float64)
        grad_beta_contrib = torch.zeros(self.M, device='cuda', dtype=torch.float64)
        grad_kappa_contrib = torch.zeros(self.M, device='cuda', dtype=torch.float64)

        self._grad_params_kernel(
            grad_alpha_total, grad_alpha_direct, grad_beta_contrib, grad_kappa_contrib,
            u_star_free, lam_free, POISSON_RATIO
        )
        ...
        return loss, grad_alpha_total, grad_alpha_direct, grad_beta_contrib, grad_kappa_contrib
...
def main():
    ...
    try:
        ...
        # Only initialize alpha as the optimization parameter
        ...
        # Compute beta and kappa from alpha using the derived relationships
        beta_init = compute_beta_from_alpha(alpha_init)
        kappa_init = compute_kappa_from_alpha(alpha_init)

        # Only alpha is a trainable parameter; beta and kappa will be computed from it
        alpha_est = torch.nn.Parameter(alpha_init)
        # Note: beta_est and kappa_est are no longer parameters, they are computed on-the-fly

        # Store initial values for comparison at the end
        alpha_initial_np = alpha_est.detach().cpu().numpy().astype(np.float64)
        beta_initial_np = beta_init.detach().cpu().numpy().astype(np.float64)
        kappa_initial_np = kappa_init.detach().cpu().numpy().astype(np.float64)
        ...
        # Only optimize alpha; beta and kappa are computed from alpha
        optimizer = torch.optim.Adam([alpha_est], lr=1.0)
        scheduler = torch.optim.lr_scheduler.MultiStepLR(
            optimizer, milestones=[100, 200, 400, 800], gamma=0.5
        )

        # Compute initial beta and kappa for statistics
        init_region_stats = _print_region_stats("Init", alpha_est, beta_init, kappa_init, log_fn=log)
        ...
        mae_beta = torch.mean(torch.abs(beta_init - beta_gt)).item()
        mae_kappa = torch.mean(torch.abs(kappa_init - kappa_gt)).item()
        ...
        for it in range(num_iterations):
            ...
            # Compute beta and kappa from alpha using the derived relationships
            beta_est = compute_beta_from_alpha(alpha_est)
            kappa_est = compute_kappa_from_alpha(alpha_est)

            sim.assemble_matrix(alpha_est, beta_est, kappa_est)
            sim.forward(tol=1e-6, max_iter=200)
            fw_status = sim.get_last_forward_status()
            loss, grad_alpha_total, grad_alpha_direct, grad_beta_contrib, grad_kappa_contrib = sim.backward(
                u_obs, tol=1e-6, max_iter=200
            )
            bw_status = sim.get_last_backward_status()

            # Gradient is already aggregated in the kernel with chain rule applied
            alpha_est.grad = grad_alpha_total
            
            # Compute percentage contributions based on relative magnitudes (sum to 100%)
            with torch.no_grad():
                norm_alpha = torch.norm(grad_alpha_direct)
                norm_beta = torch.norm(grad_beta_contrib)
                norm_kappa = torch.norm(grad_kappa_contrib)
                total_of_norms = norm_alpha + norm_beta + norm_kappa
                if total_of_norms > 1e-12:
                    pct_alpha = 100.0 * norm_alpha / total_of_norms
                    pct_beta = 100.0 * norm_beta / total_of_norms
                    pct_kappa = 100.0 * norm_kappa / total_of_norms
                else:
                    pct_alpha = pct_beta = pct_kappa = 0.0

            optimizer.step()
            scheduler.step()

            with torch.no_grad():
                # Recompute beta and kappa for error tracking (they are derived from updated alpha)
                beta_est_current = compute_beta_from_alpha(alpha_est)
                kappa_est_current = compute_kappa_from_alpha(alpha_est)
                
                err_alpha = torch.mean(
                    torch.abs(alpha_est - alpha_gt) / (torch.abs(alpha_gt) + 1e-6)
                ).item()
                err_beta = torch.mean(
                    torch.abs(beta_est_current - beta_gt) / (torch.abs(beta_gt) + 1e-6)
                ).item()
                err_kappa = torch.mean(
                    torch.abs(kappa_est_current - kappa_gt) / (torch.abs(kappa_gt) + 1e-6)
                ).item()
                mae_alpha_iter = torch.mean(torch.abs(alpha_est - alpha_gt)).item()
                mae_beta_iter = torch.mean(torch.abs(beta_est_current - beta_gt)).item()
                mae_kappa_iter = torch.mean(torch.abs(kappa_est_current - kappa_gt)).item()

            # Only alpha has a learning rate now
            ...
            log(
                f"  Gradient contributions: "
                f"direct_alpha={pct_alpha:.1f}%, via_beta={pct_beta:.1f}%, via_kappa={pct_kappa:.1f}%"
            )
            # Use the current derived values for statistics
            iter_region_stats = _print_region_stats(
                f"Iter {it+1:03d}", alpha_est, beta_est_current, kappa_est_current, log_fn=log
            )
            history_entries.append(
                {
                    "iteration": it + 1,
                    ...
                    "learning_rate": lr_alpha, 
                    "gradient_contributions_pct": {
                        "direct_alpha": float(pct_alpha),
                        "via_beta": float(pct_beta),
                        "via_kappa": float(pct_kappa),
                    },
        ...
        with torch.no_grad():
            ...
            beta_est_final = compute_beta_from_alpha(alpha_est)
            kappa_est_final = compute_kappa_from_alpha(alpha_est)
            
            mae_alpha = torch.mean(torch.abs(alpha_est - alpha_gt)).item()
            mae_beta = torch.mean(torch.abs(beta_est_final - beta_gt)).item()
            mae_kappa = torch.mean(torch.abs(kappa_est_final - kappa_gt)).item()
            save_parameter_heatmap(sim, alpha_est, final_params_path, labels_np, log_fn=log)
        final_region_stats = _print_region_stats("Final", alpha_est, beta_est_final, kappa_est_final, log_fn=log)
        log("Optimization complete.")
        log(f"MAE alpha={mae_alpha:.6e}, beta={mae_beta:.6e}, kappa={mae_kappa:.6e}")

        alpha_final_np = alpha_est.detach().cpu().numpy().astype(np.float64)
        beta_final_np = beta_est_final.detach().cpu().numpy().astype(np.float64)
        kappa_final_np = kappa_est_final.detach().cpu().numpy().astype(np.float64)
